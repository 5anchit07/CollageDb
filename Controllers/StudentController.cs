...
namespace MongoCrud.API.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        public StudentController(IMapper mapper, IStudentRepository studentRepo,IBranchRepository branchRepo)
        {
            _mapper = mapper;
        ...
        public async Task<IActionResult> Create([FromBody] AddStudentRequestDto studentDto)
        {
            ...
            var branch=await _branchRepo.GetByBranchIdAsync(studentDto.BranchId);
            if(branch == null)
            {
                return NotFound("the branch id not matched");
            }
            // dto to model
        ...
        [HttpGet("with-branch-details")]
        public async Task<IActionResult> GetAllStudentWithDetail()
        {
            var student = await _studentRepo.GetAllWithBranchAsync();
            return Ok(student);
        }
